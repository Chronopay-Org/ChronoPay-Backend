 ...
 */
export function detectDrift(registered: Migration[], applied: AppliedMigration[]): DriftResult {
  const errors: string[] = [];
...
export function validateMigrationOrder(migrations: Migration[]): DriftResult {
  ...
  for (let i = 0; i < migrations.length; i++) {
    ...
    if (!/^[a-z][a-z0-9_]*$/.test(m.name)) {
      warnings.push(`Migration "${m.id}" name "${m.name}" should use snake_case convention`);
    }
