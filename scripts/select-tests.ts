...
export type TestGraph = Record<string, string[]>;

export type SelectResult =
  | { kind: "full_run"; reason: string }
  | { kind: "selected"; tests: string[] };
 ...
 */
export function loadGraph(graphPath: string): TestGraph {
  const raw = fs.readFileSync(graphPath, "utf-8");
  const parsed: unknown = JSON.parse(raw);
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
    throw new Error(
      `test-graph.json must be a JSON object, got ${Array.isArray(parsed) ? "array" : typeof parsed}`,
    );
  }
  return parsed as TestGraph;
}
 ...
 */
export function filterExisting(tests: string[], repoRoot: string): string[] {
...
/* istanbul ignore next */
function main(): void {
  const { changedFiles, graphPath, output, verbose } = parseArgs(
  ...
  if (verbose) {
    if (result.kind === "full_run") {
      ...
      console.error(
        `[select-tests] selected ${result.tests.length} test file(s)`,
      );
    }
  }

  const text =
    result.kind === "full_run"
      ? FULL_RUN_SENTINEL
      : result.tests.join("\n");

  if (output) {
    fs.writeFileSync(output, text + "\n", "utf-8");
    if (verbose) console.error(`[select-tests] wrote output to ${output}`);
  } else {
