...
export async function runCanary() {
  ...
  try {
    ...
    if (!createIntentRes.ok) throw new Error(`Failed to create intent: ${createIntentRes.status}`);
    
    const intentData = await createIntentRes.json();
    createdIntentId = intentData.intent?.id;
    if (!createdIntentId) throw new Error("No intent ID returned");
    ...
    if (!confirmRes.ok) throw new Error(`Failed to confirm intent: ${confirmRes.status}`);
    ...
    if (!cancelRes.ok) throw new Error(`Failed to cancel intent: ${cancelRes.status}`);

    if (redis) await redis.del(STALE_KEY); // Clean up the stale marker
